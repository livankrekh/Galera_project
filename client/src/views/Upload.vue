<template>
  <section class="screen-height screen-section">
    <div class="overlay"/>
    <div class="justify-content-center substrate">
      <div id="uploadPanel" class="text-center">
        <h1 class="caption" v-bind:class="{ caption_small: state !== State.IDLE }">オンライン商品認識システム</h1>
        <div>
          <!--Select file button-->
          <button class="btn btn-primary btn-big"
                  @click="onPickFile"
                  v-if="state === State.IDLE">ファイルを選択
          </button>
          <div class="d-flex align-items-center result-panel">
            <!--Image-->
            <div
                class="image-container text-center m-2"
                v-if="state !== State.IDLE && state !== State.RESULT">
            ...
            <div v-if="state === State.RESULT" class="image-list block-container result-container">
              <h3 class="text-center caption caption_small">結果</h3>
              <div class="d-flex text-left justify-content-center result-text">
                <div class="mr-3">
                  <div>項目１</div>
                  <div>項目２</div>
                  <div>項目３</div>
                ...
                <div class="flex-wrap">
                  <div class="flex-grow-1">情報情報情報１</div>
                  <div class="flex-grow-1">情報情報情報２</div>
                  <div class="flex-grow-1">情報情報３</div>
                  <div class="flex-grow-1">情報情報情報情報４</div>
                  <div class="flex-grow-1">情報５</div>
                </div>
              </div>
            </div>
            <!--Do not touch -->
            ...
            <div class="image-list block-container result-container"
                 v-if="state === State.RECOGNITION_RESULT">
              <h3 class="text-center caption caption_small">認識結果</h3>
              <div class="d-flex text-left justify-content-center result-text">
                <div class="mr-3">
                  <div>項目１</div>
                  <div>項目２</div>
                ...
                <div class="flex-wrap">
                  <div class="flex-grow-1">情報情報１</div>
                  <div class="flex-grow-1">情報情報情報２</div>
                  <div class="flex-grow-1">情報３</div>
                  <div class="flex-grow-1">情報情報情報情報４</div>
                  <div class="flex-grow-1">情報情報情報５</div>
                </div>
              </div>
            </div>
            <div class="position-relative image-list block-container result-container align-items-stretch"
                 style="overflow-y: auto;"
                 v-if="state === State.RERECOGNITION_RESULT">
              <h3 class="text-center caption caption_small" style="padding-top: 0">再認識結果</h3>
              <div v-for="item in items" class="d-flex text-left result_item mb-2 justify-content-center result-text"
                   v-bind:class="{ item_selected: item.isActive }"
                   @click="item.isActive=true; selectedItem.isActive=false; selectedItem=item">
                <div class="mr-3">
                  <img class="show-img img-responsive p-2 result-item-img"
                       :src="imagePreview"
                       alt="image"/>
                </div>
                <div class="mr-3">
                ...
                <div class="flex-wrap">
                  <div class="">情報情報情報情報</div>
                  <div class="">情報情報情報情報情報情報</div>
                  <div class="">情報情報情報情</div>
                  <div class="">情報</div>
                  <div class="">情報情報情報情報情報情</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="flex mt-4" v-if="images.length">
            ...
            <button
                ...
                type="button"
                class="btn btn-primary"
                v-on:click="upload">認識
            </button>
            <button
                v-if="state===State.RECOGNITION_RESULT"
                type="button"
                class="btn btn-primary"
                v-on:click="redetect">再認識
            </button>
            <button
                v-if="state===State.RERECOGNITION_RESULT"
                type="button"
                class="btn btn-primary"
                v-on:click="goToManual">手動入力
            </button>
            <button
                v-if="state===State.IMAGE_SELECTED ||
          ...
          state===State.RERECOGNITION_RESULT"
                type="button"
                class="btn btn-primary"
                @click.prevent="deleteImage(currentIndexImage)">取り消し
            </button>
            <button
                v-if="state===State.RESULT"
                type="button"
                class="btn btn-primary"
                @click.prevent="deleteImage(currentIndexImage)">戻る
            </button>
          </div>
          <!--Alert-->
...
export default {
  ...
  methods: {
    ...
    upload() {
      ...
      // axios.post('http://localhost/api/predict', image, headers: {
    ...
    goToManual() {
      this.state = State.MANUAL_INPUT_FORM
    },
    preventEvent(e) {
      e.preventDefault()
      e.stopPropagation()
...
@font-face {
  ...
  url("/src/assets/yuminl.ttf") format("truetype");
}

@font-face {
  font-family: "Yu Mincho Regular";
  src: local("Yu Mincho Regular"),
  url("/src/assets/yumin.ttf") format("truetype");
}
